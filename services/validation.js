...
function validateTransaction(payload) {
  ...
  const merchantAverageAmount = optionalNumber(payload.merchant_average_amount);
  ...
  const merchantRiskScore = optionalNumber(payload.merchant_risk_score);
  ...
  if (
    merchantAverageAmount !== null &&
    (!Number.isFinite(merchantAverageAmount) || merchantAverageAmount <= 0)
  ) {
    errors.push("merchant_average_amount must be greater than 0 when provided");
  }
  ...
  if (transactionStatus && !VALID_TRANSACTION_STATUSES.includes(transactionStatus)) {
    errors.push(
      `transaction_status must be one of: ${VALID_TRANSACTION_STATUSES.join(", ")}`
    );
  }
  ...
  if (
    merchantRiskScore !== null &&
    (!Number.isInteger(merchantRiskScore) || merchantRiskScore < 0 || merchantRiskScore > 100)
  ) {
    errors.push("merchant_risk_score must be a whole number from 0 to 100");
  }
